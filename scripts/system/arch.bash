...
function system_arch_base_system() {
	...
	pacman --noconfirm -Syyu && pacman --noconfirm -Sy "${security[@]}" "${build[@]}" "${net[@]}" "${files[@]}"
}
...
function system_arch_qol() {
	local -ra packages=("bash-completion" "man-db")
	pacman --noconfirm -Syyu && pacman --noconfirm -Sy "${packages[@]}"
}
